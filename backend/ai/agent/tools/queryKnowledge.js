...
`;
            ...
            try {
                ...
                answer = genResult.response.text()?.trim() || "No detailed answer generated.";
            } catch (llmErr) {
                console.warn("Gemini knowledge synthesis fallback:", llmErr.message);
                answer = `Found ${documents.length} document(s) matching your inquiry: ${documents.map(d => d.title).join(", ")}. Document excerpt:\n${documents[0].content?.slice(0, 300) || "No excerpt text available."}`;
            }
